...
const Chord = (props) => {
  ...
  const displayChord = () => {
    return (
      MID_NOTES[chord.name] +
      CHORD_QUALITIES[chord.quality] +
      '/' +
      BASS_NOTES[chord.bass]
    );
  };

  const submitChange = () => {
    dispatch(
      changeSingleChord(id, { bass: bass, name: name, quality: quality })
    );
    toggleEditMode();
  };
    ...
    <>
      <div className={chordIsPlaying() ? 'chord-box playing' : 'chord-box'}>
        {editMode ? (
          ...
          <>
            <span className='chord-name'>{displayChord()}</span>
            <FontAwesomeIcon
